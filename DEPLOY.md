...
## Черновик новой главной `/home2` (только админ)

Песочница под редизайн главной. Боевая `/` не меняется вообще — это отдельный
роут со своим рендером (`app.js`, префикс `h2-*`) и своим блоком стилей в конце
`styles.css`. Никаких переменных окружения: доступ решает `ADMIN_USER_IDS`
(гейт во фронте + `noindex` на странице), поэтому включать/выключать нечего.

- Вход: пункт меню **◈ ГЛАВНАЯ v2** (виден только админу) или `/dev` → `ГЛАВНАЯ v2 ↗`.
- Два макета переключаются кнопками на самой странице, выбор липнет в
  localStorage (`sz_home2`). Прямая ссылка на конкретный: `/home2?lay=hud`,
  `/home2?lay=board`.
  - **ПУЛЬТ** — приборы слева, главный экран по центру, новости справа; вместо
    рамок угловые скобы.
  - **ПЛАТА** — центральная шина новостей, модули-компоненты по краям на
    дорожках, фон-миллиметровка, срезанные углы.
- Реклама на странице не показывается (её нет в `AD_BOTTOM_PATHS`) — как и на `/dev/*`.
