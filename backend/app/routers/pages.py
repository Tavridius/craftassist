...
def _market_seo() -> tuple[str, dict]:
    return _seo_block(
        ...
        faq=[
            ...
            # формулировка под запрос из Вебмастера: «какая комиссия у аукциона в
            # старкрафте» — 1747 показов на 4-й позиции и 0 кликов (сентябрь 2026)
            ("Какая комиссия у аукциона в Сталкрафт?",
             "Комиссия аукциона Сталкрафт — 5% с продажи: её платит продавец, когда лот "
             "выкупают. Выставить лот 100 000 ₽ — значит получить 95 000 ₽. Во всех расчётах "
             "выгоды на сайте (калькуляторы крафта, бартера и обменок) эти 5% уже вычтены."),
        ],
