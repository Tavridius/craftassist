...
def _ceil(key: str) -> float:
    ...
    значение, на котором урон уже идёт (подтверждено владельцем 15.08.2026),
    поэтому держим микрозазор. Оптимизатор здесь knapsack: без зазора он охотно
    садится ровно на лимит, и сборка на ровном лимите уезжала игроку помеченной
    как безопасная. Ограничения «минус не в
